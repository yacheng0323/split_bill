...
class _NewMemberPageState extends ConsumerState<NewMemberPage> {
  ...
  @override
  void initState() {
    ref.read(newMemberViewModelProvider.notifier).getMembers(widget.tableId);
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(newMemberViewModelProvider);
    return state.when(
      data: (data) {
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          onPressed: controller.text.isEmpty
                              ? null
                              : () async {
                                  members.contains(controller.text)
                                      ? ShowSnackBarHelper.errorSnackBar(
                                              context: context)
                                          .showSnackbar(
                                              "Member already exists.")
                                      : await ref
                                          .read(newMemberViewModelProvider
                                              .notifier)
                                          .addMember(
