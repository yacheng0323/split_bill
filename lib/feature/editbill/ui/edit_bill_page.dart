...
class _EditBillPageState extends ConsumerState<EditBillPage> {
  ...
  late final amountController = TextEditingController(
      text: "\$${widget.billModel.money.toStringAsFixed(0)}");
  ...
  void initState() {
    ref
        .read(editBillViewModelProvider.notifier)
        .init(widget.tableId, widget.billModel.settledBy);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    final editBillState = ref.watch(editBillViewModelProvider);

    return editBillState.when(
      data: (data) {
        final members = data.members ?? [];
        final settledMembers = data.settledMembers ?? [];
        return Scaffold(
                              ...
                              child: TextFormField(
                                ...
                                onTap: () {
                                  DatePicker.showDatePicker(
                                    ...
                                    onConfirm: (date) {
                                      ref
                                          .read(editBillViewModelProvider
                                              .notifier)
                                          .setDateTime(date);
                                      setState(() {
                                        dateController.text =
                                              ...
                                              return ListTile(
                                                leading: Checkbox(
                                                  ...
                                                  onChanged: (value) {
                                                    ref
                                                        .read(
                                                            editBillViewModelProvider
                                                                .notifier)
                                                        .toggleMember(
                                                            members[index]);
                                                    setState(() {
                                                      settledByController.text =
                                                          settledMembers.isEmpty
                                                              ? ""
                                                              : settledMembers
                                                                  .map((e) => e)
                                                                  .toString();
